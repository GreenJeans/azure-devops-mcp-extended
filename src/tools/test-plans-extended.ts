...
function configureExtendedTestPlanTools(server: McpServer, connectionProvider: () => Promise<WebApi>) {
  /*
    LIST TEST SUITES
    get all test suites for a plan
  */
  server.tool(
    Extended_Test_Plan_Tools.list_test_suites,
    "Retrieve all test suites for a given test plan, optionally as a tree view. Returns suite hierarchy including suite type (static, requirement-based, query-based), parent relationships, and child counts.",
    {
  ...
  */
  server.tool(
    Extended_Test_Plan_Tools.get_test_points,
    "Retrieve test points for a test suite. Test points represent the assignment of a test case to a configuration and tester, and include the latest execution outcome (passed, failed, blocked, not applicable, paused, in progress, none).",
    {
      project: z.string().describe("The unique identifier (ID or name) of the Azure DevOps project."),
      planId: z.number().describe("The ID of the test plan."),
  ...
  */
  server.tool(
    Extended_Test_Plan_Tools.list_test_runs,
    "Retrieve test runs for a project. Test runs are created when tests are executed via the Test Runner or automated pipelines. Returns run details including state, total/passed/failed counts, and associated plan/build information.",
    {
      project: z.string().describe("The unique identifier (ID or name) of the Azure DevOps project."),
